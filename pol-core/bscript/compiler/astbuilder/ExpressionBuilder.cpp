    ...
    std::unique_ptr<Expression> lhs, EscriptParser::ExpressionListContext* ctx )
{
  ...
  auto xx = std::make_unique<ArrayElementIndexes>( location_for( *ctx ),
                                                   std::move( indexes ) );

  return std::make_unique<ArrayElementAccess>( source_location, std::move( lhs ), std::move( xx ) );
}
    ...
    std::unique_ptr<Expression> lhs, EscriptParser::ExpressionSuffixContext* ctx )
{
  ...
  {
    return array_access( std::move( lhs ), indexing->expressionList() );
  }
